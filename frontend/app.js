const App = (() => {
  ...
  function navigate(route) {
    ...
    if (route === "head-kp") { loadKpList(); }
    if (route === "kp") { loadKpList(); }
  }
...
<div class="cr-actions">
  ...
  /* ============ КОНСТРУКТОР КП (kp-service) ============ */
  async function loadKpList() {
    const isHead = (getUser() || {}).role === "head";
    const wrap = $(isHead ? "#kp-list-head" : "#kp-list") || $("#kp-list") || $("#kp-list-head");
    if (!wrap) return;
    wrap.innerHTML = "Загрузка…";
    try {
  ...
  async function createKp() {
    try {
      const doc = await api("/api/kp/documents", { method: "POST", body: { title: "Коммерческое предложение", payload: {} } });
      location.href = "/kp.html?id=" + doc.id;
    } catch (e) { toast("Не удалось создать КП: " + e.message, "error"); }
  }
  ...
  async function deleteKp(id, title) {
    ...
    try {
      await api(`/api/kp/documents/${id}`, { method: "DELETE" });
      toast("КП удалено", "success");
      loadKpList();
  ...
  async function init() {
    ...
    if (getToken() && getUser()) {
      ...
      if (pendingRoute) {
        localStorage.removeItem("bitserves_pending_route");
        if ((getUser() || {}).role === "head" || !pendingRoute.startsWith("head-")) navigate(pendingRoute);
      }
    }
    else showLogin();
  }
