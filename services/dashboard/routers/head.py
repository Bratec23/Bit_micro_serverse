...
from app.clients import get_department_members, get_latest_records, get_user_profile
...
def get_current_head(token: str = Depends(oauth2_scheme)) -> dict:
...
def _calc_metrics(rec: dict, cost_price: float = 0.0) -> dict:
    ...
    return {
        ...
        "kpi2_revenue": float(rec.get("kpi2_revenue", 0)),
        "kpi2_bonus_amount": float(rec.get("kpi2_bonus_amount", 0)),
    }
...
def _fetch_department_data(head: dict, periods: list[str]) -> tuple[list[dict], dict]:
    members = get_department_members(head["department_id"], role="manager")
    user_ids = [m["id"] for m in members]
    if user_ids:
        data = get_latest_records(periods, user_ids)
    else:
        data = {"records": {}, "cost_prices": {}}
    return members, data


def _record_for(data: dict, user_id: int, period: str) -> Optional[dict]:
    return data.get("records", {}).get(str(user_id), {}).get(period)


def _cost_price_for(data: dict, user_id: int, period: str) -> float:
    return float(data.get("cost_prices", {}).get(str(user_id), {}).get(period, 0.0))
...
def dashboard(period: str = Query(...), head: dict = Depends(get_current_head)):
    dept_name = head.get("department_name") or "—"
    members_q, data = _fetch_department_data(head, [period])
    members: list[TeamMemberOut] = []
    metrics: list[MetricsRowOut] = []
    totals = {
    ...
    for m in members_q:
        rec = _record_for(data, m["id"], period)
        rec_dict = None
        if rec:
            rec_dict = {
                "id": rec["id"], "period": rec["period"],
                "service_margin": rec["service_margin"],
                "goods_margin": rec["goods_margin"],
                "bonus_total": rec["bonus_total"],
                "bonus_total_with_kpi2": rec["bonus_total_with_kpi2"],
                "gross_pay": rec["gross_pay"],
                "tax_amount": rec["tax_amount"],
                "net_pay": rec["net_pay"],
                "base_salary": rec["base_salary"],
                "kpi2_revenue": rec["kpi2_revenue"],
                "kpi2_bonus_amount": rec["kpi2_bonus_amount"],
                "kpi2_paid": rec["kpi2_paid"],
                "created_at": rec["created_at"],
            }
        members.append(TeamMemberOut(
            user_id=m["id"], full_name=m["full_name"],
            position_name=m.get("position_name") or "—",
            grade_name=m.get("grade_name"),
            base_salary=m.get("base_salary"),
            record=rec_dict,
        ))
        if rec:
            cp = _cost_price_for(data, m["id"], period)
            ...
            metrics.append(MetricsRowOut(
                ...
                **cm, fot_status=_fot_status(cm["fot_margin_pct"]),
                kpi2_paid=bool(rec.get("kpi2_paid")), kpi2_retention_pct=float(rec.get("kpi2_retention_pct", 0)),
            ))
...
def profitability(payload: ProfitabilityRequest, head: dict = Depends(get_current_head)):
    dept_name = head.get("department_name") or "—"
    members_q, data = _fetch_department_data(head, [payload.period])
    members_by_id = {m["id"]: m for m in members_q}
    rows: list[ProfitabilityRowOut] = []
    totals = {
        "margin": 0.0, "margin_net": 0.0, "gross": 0.0, "ndfl": 0.0, "insurance": 0.0, "vat": 0.0,
        "office": 0.0, "labor_cost": 0.0, "operating_cost": 0.0, "total_cost": 0.0,
    ...
    for item in payload.items:
        member = members_by_id.get(item.user_id)
        if not member:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                                detail=f"Менеджер {item.user_id} не найден или не входит в ваш отдел")
        rec = _record_for(data, item.user_id, payload.period)
        if not rec:
            rows.append(ProfitabilityRowOut(
                user_id=member["id"], full_name=member["full_name"], cost_price=item.cost_price,
            ...
            continue
        cp = item.cost_price if item.cost_price > 0 else _cost_price_for(data, item.user_id, payload.period)
        cm = _calc_metrics(rec, cp)
        cm.pop("cost_price", None)
        rows.append(ProfitabilityRowOut(
            user_id=member["id"], full_name=member["full_name"], cost_price=cp,
...
def costs(period: str = Query(...), head: dict = Depends(get_current_head)):
    dept_name = head.get("department_name") or "—"
    managers, data = _fetch_department_data(head, [period])
    items = []
    totals = {
...
def history(from_period: str = Query(..., alias="from"), to_period: str = Query(..., alias="to"),
            head: dict = Depends(get_current_head)):
    dept_name = head.get("department_name") or "—"
    periods = _period_range(from_period, to_period)
    managers, data = _fetch_department_data(head, periods)
    by_manager = []
    monthly = []
    for p in periods:
        margins = 0.0; margins_net = 0.0; gross = 0.0; profit = 0.0; fot_pct_sum = 0.0; fot_count = 0
        for m in managers:
            rec = _record_for(data, m["id"], p)
            if rec:
                cp = _cost_price_for(data, m["id"], p)
                cm = _calc_metrics(rec, cp)
...
def waterfall(period: str = Query(...), head: dict = Depends(get_current_head)):
    prev_p = _prev_period(period)
    managers, data = _fetch_department_data(head, [period, prev_p])
    items: list[WaterfallItemOut] = []
    prev_total = 0.0
    cur_total = 0.0
    for m in managers:
